...
module.exports = {
    ...
    create(context) {
         ...
         * @returns {ASTNode} node that has indent errors
         */
        function getErrorNode(node) {
            const nodeFound = errorNodes.find(function(nodeObj) {
                return nodeObj.node === node;
            });
        ...
        function blockIndentationCheck(node) {
            ...
            if (node.parent && statementsWithProperties.indexOf(node.parent.type) !== -1 && isNodeBodyBlock(node)) {
                errorNode = getErrorNode(node.parent);

                if (errorNode) {
                    indent = errorNode.needed;
                } else {
